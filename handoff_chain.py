"""
handoff_chain.py — OpenAI Agents SDK 三 Agent round-trip handoff demo
...
api_key = os.environ.get("OPENAI_API_KEY") or "sk-kynuspdhaccjcdkysiznyndssejxnhfyvhcaxpbikjwaiffr"
...
# Create SiliconFlow-compatible Chat Completions model with DeepSeek
...
""",
    ...
    async def run_tests():
        ...
        result = await Runner.run(router_agent, "给我讲讲递归是什么", max_turns=5, run_config=run_config)
        ...
        result = await Runner.run(router_agent, "帮我想一个斐波那契的 Python 实现方案", max_turns=5, run_config=run_config)
        print(f"Result: {result.final_output}")
        print(f"Last Agent: {result.last_agent.name if result.last_agent else 'none'}")
        print()
        ...
        result = await Runner.run(router_agent, "帮我把 'Hello, World!' 写到 hello.txt 文件", max_turns=5, run_config=run_config)
        print(f"Result: {result.final_output}")
        print(f"Last Agent: {result.last_agent.name if result.last_agent else 'none'}")
        print()
